...
# reads one word left-context, focus, and one word right-context trigrams
        ...
        if self.trainFile:
            with open(self.trainFile, 'r') as f:
                for line in f:
                    ...
                    words = line.split()
                    focusWord = words[1].split(None, 1)[0]
                    ...
                    n2Tuple = self.id2word.doc2bow(focusWordListForm)
                    ...
                    if n1Tuple and n2Tuple and n3Tuple:
                        ...
                        lcStart = time.time()
                        #lc = getCachedLSAConf(lastWord,n3Id,mm)
                        lc = self.getPrecachedLSAConf(n2Id)
                        self.condPrint(5, "  < Done computing the LSAconf in %f" % (time.time() - lcStart))
            
                        outputString = "%.16f\t%s %s %s\t%.16f" % (PLvalue, words[0], focusWord, words[2], lc)
                        if self.outputFile:
